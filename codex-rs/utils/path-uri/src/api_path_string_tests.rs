...
fn other_non_absolute_api_paths_cannot_be_converted_to_path_uris() {
    for raw_path in [r"workspace\file.rs", r"C:file.rs"] {
        let path = serde_json::from_value::<LegacyAppPathString>(serde_json::json!(raw_path))
...
fn converts_absolute_api_paths_using_the_inferred_convention() {
    ...
    assert_eq!(
        ...
        Some(PathUri::parse("file:///workspace/file.rs").expect("expected URI should parse")),
    );
...
fn ambiguous_absolute_api_paths_preserve_their_inferred_convention() {
    ...
    assert_eq!(uri.infer_path_convention(), Some(PathConvention::Posix));
...
fn converts_native_api_path_to_inferred_absolute_path() {
    ...
    if let Ok(expected) = AbsolutePathBuf::from_absolute_path_checked(raw_path) {
        assert_eq!(AbsolutePathBuf::try_from(path.clone()), Ok(expected.clone()));
...
fn foreign_absolute_syntax_deserializes_without_host_interpretation() {
    for (raw_path, convention) in [
        ...
        assert_eq!(path.infer_absolute_path_convention(), convention);
    }
}
...
fn from_path_preserves_foreign_absolute_path_for_uri_conversion() {
    let (foreign_path, expected_uri) =
        (r"C:\Users\openai\share", "file:///C:/Users/openai/share");

    let converted = LegacyAppPathString::from_path(std::path::Path::new(foreign_path))
        .to_inferred_path_uri();
    if let Some(path) = converted {
        assert_eq!(path, PathUri::parse(expected_uri).expect("valid expected URI"));
    }
}
