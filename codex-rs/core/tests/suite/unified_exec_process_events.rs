...
async fn respond_environment_info(websocket: &mut WebSocketStream<TcpStream>, id: &Value) {
    let shell = json!({ "name": "zsh", "path": "/bin/zsh" });
    send_exec_server_json(
        websocket,
        json!({
            "id": id,
...
#[test_case(PushedExecScenario::Complete, ManagedNetworkScenario::None ; "complete_event_stream")]
#[test_case(PushedExecScenario::DirectDenied, ManagedNetworkScenario::None ; "direct_sandbox_denial")]
#[test_case(PushedExecScenario::LegacyExit, ManagedNetworkScenario::None ; "legacy_exit_metadata")]
#[test_case(PushedExecScenario::ReplayGap, ManagedNetworkScenario::None ; "truncated_event_replay")]
#[test_case(PushedExecScenario::Complete, ManagedNetworkScenario::Enabled { policy_callbacks: true } ; "managed_network_uses_executor_proxy_launch")]
#[test_case(PushedExecScenario::Complete, ManagedNetworkScenario::Enabled { policy_callbacks: false } ; "strict_managed_allowlist_omits_policy_callbacks")]
#[test_case(PushedExecScenario::Complete, ManagedNetworkScenario::Disabled ; "disabled_managed_network_omits_executor_proxy_launch")]
...
async fn exec_command_consumes_pushed_remote_process_events(
    ...
    let tool_call = ev_function_call(
        CALL_ID,
        "exec_command",
        &json!({
            "cmd": "pwd",
    ...
    test.codex
        .start_or_steer_turn(
            ...
            .with_thread_settings(ThreadSettingsOverrides {
                environments: None,
                approval_policy: Some(if managed_network_configured {
